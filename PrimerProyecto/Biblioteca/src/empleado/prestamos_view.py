...
# IMPORTANTE: El nombre de la función DEBE ser prestamos_tab
def prestamos_tab(page: ft.Page) -> ft.Control:
    ...
    # --- TABLA CON COLUMNA DE FECHA AGREGADA ---
    tabla = ft.DataTable(
        column_spacing=20,
        columns=[
            ft.DataColumn(ft.Text("Lector")), 
            ft.DataColumn(ft.Text("Libro")),
            ft.DataColumn(ft.Text("Vence")), # <-- NUEVA COLUMNA
    ...
    def inicializar_data():
        ...
        filtrar_catalogos() # Llena lectores
        refrescar_tabla()
        page.update()
    ...
    listado = ft.Container(
        ...
        content=ft.Column(
            expand=True, 
            scroll=ft.ScrollMode.ALWAYS, # Solo permitimos scroll en Column
            controls=[
                ft.Text("Control Devoluciones", size=18, weight="bold"),
                txt_buscar_dev,
                ft.Row([tabla], scroll=ft.ScrollMode.ALWAYS) # Tabla con scroll lateral
            ]
        )
    )
